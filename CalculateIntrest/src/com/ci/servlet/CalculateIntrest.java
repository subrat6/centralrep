...
import com.ci.beans.PropertiesConst;
...
public class CalculateIntrest extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		/*
		 * if (cache.containsKey(ri)==false) { props.load(new
		 * FileInputStream(request.getServletContext().getRealPath(
		 * "/WEB-INF/intrest.properties")));
		 * //props.load(request.getServletContext().getResourceAsStream(
		 * "/WEB-INF/intrest.properties")); cache.put(ri, props); }else {
		 */
